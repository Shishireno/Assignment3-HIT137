...
self.differences = []	Clears state from any previous round, ensuring a clean reset on new image load
...
Returns True if new_box overlaps any rectangle in boxes, using a 35 px padding buffer to ensure differences are visually separated.
...
Uses axis-aligned bounding-box separation testing. A new box is only accepted if it is separated from every existing box by at least 35 pixels on all axes, preventing two alterations from being close enough to be confused for one another.

create_differences(self)

Iterates up to 1,500 times attempting to place five non-overlapping alterations. On each iteration:

Samples a random box size between 55–95 px (clamped to image_dimension / 6 for small images)
Samples a random position keeping the box at least 20 px from every image edge
Calls check_overlap() — skips the position if it collides with an existing box
Selects an alteration type at uniform random from all seven available types
Calls apply_difference() to write the alteration to self.modified
Appends a Difference object to self.differences

The 1,500-attempt cap guarantees the loop always terminates. On typical photographs this limit is never reached.

apply_difference(self, x, y, w, h, diff_type)

Dispatches to the correct pixel-manipulation branch for the given diff_type and applies it directly to self.modified.

Type	OpenCV Technique	Visual Effect	Difficulty
colour_shift	cv2.add / cv2.subtract on B and G channels	Blue channel +45, green −25 — cold, desaturated patch	Medium
blur	cv2.GaussianBlur with 21×21 kernel	Soft, out-of-focus rectangle	Low–Medium
brightness	convertScaleAbs alpha=1.15, beta=45	Region brightened by scaling then offset	Low
dark_patch	convertScaleAbs alpha=0.55, beta=0	Region darkened to 55% of original luminance	Low–Medium
edge_line	cv2.line horizontal across centre	Near-black 4 px horizontal line	High
tiny_symbol	cv2.circle + cv2.line cross-hair	Dark circle with horizontal bar	High
texture_noise	cv2.add / cv2.subtract on R and B channels	Red channel boosted, blue reduced — warm colour cast	Medium–High
